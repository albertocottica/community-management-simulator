...
##                    if ob["globalchattiness"] == c and  ob["intimacystrength"] == i and ob["policy"] == pt:
...
##                        ms_x2 = float(thisCase["newer_ms_avg_gini"])
##                        ms_s2 = float(thisCase["newer_ms_se_gini"])
##                        ms_x1 = float(thisCase["more active_ms_avg_gini"])
##                        ms_s1 = float(thisCase["more active_ms_se_gini"])
##                        ms_t = (ms_x1 - ms_x2) / (((ms_s1 ** 2 + ms_s2 ** 2) /24) ** (0.5))
##                        thisCase["ms_t_more active_newer"] = ms_t
##
##                        nc_x2 = float(thisCase["newer_nc_avg_gini"])
##                        nc_s2 = float(thisCase["newer_nc_se_gini"])
##                        nc_x1 = float(thisCase["more active_nc_avg_gini"])
##                        nc_s1 = float(thisCase["more active_nc_se_gini"])
##                        nc_t = (nc_x1 - nc_x2) / (((nc_s1 ** 2  + nc_s2 ** 2) /24) ** (0.5))
##                        thisCase["nc_t_more active_newer"] = nc_t
##
...
if __name__ == "__main__":
    ...
    for item in results:
        print ("globalchattiness: " + str(item["globalchattiness"]) + "; intimacy strength: " + str(item["intimacystrength"]) + "; policy: " + str(item["policy"]))
