...
def rename_vars(table):
    ...
    for row in table:
        for key in row:
            newkey = key.replace('engage_vs_none', 'evn')
...
def normalise(table):
    ...
    '''
    to_normalise = ['dropouts_evn', 'nc_t_evn', 'ms_t_evn', 'mgmteffort_evn']
    for row in table: 
        for key in row:
...
if __name__ == '__main__':
    ...
    data = readFile(dirPath + 'results_experiment_2_2.csv')
    data1 = destring(data)
    data2 = rename_vars(data1)
    print data2[0]
    data3 = normalise(data2)
    print data3[0]
    success = writeFile(data3, dirPath + 'norm_results_experiment_2.2.csv')
