...
def rename_vars(table):
    ...
    for row in table:
        for key in row:
            ...
            newnewnewkey = newkey.replace('onboard_vs_none', 'ovn')
            row[newnewnewkey] = row.pop(key)
    return table
...
def normalise(table):
    ...
    '''
    to_normalise = ['dropouts_ovn', 'nc_t_ovn', 'ms_t_ovn', 'mgmteffort_ovn']
    for row in table: 
        for key in row:
...
if __name__ == '__main__':
    ...
    data = readFile(dirPath + 'results_experiment_2_1.csv')
    data0 = destring(data)
    data1 = rename_vars(data0)
    data2 = normalise(data1)
    lookVal = lookup(data2)
    normalised_results = writeFile(data2, dirPath + 'norm_results_experiment_2_1.csv' )
