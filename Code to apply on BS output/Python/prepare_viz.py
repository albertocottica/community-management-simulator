    ...
    for row in table:
        for key in row:
            ...
            if oldval == '':
                ...
                newval = False
            elif oldval in ["newer", "more active", 'engage', 'both']: 
                continue
            else:
...
def rename_vars(table):
    ...
    for row in table:
        for key in row:
            newkey = key.replace('both_vs_engage', 'bve')
            newnewkey = newkey.replace('more_active_vs_newer', 'mvn')
            row[newnewkey] = row.pop(key)
    return table
...
def normalise(table):
    ...
    '''
    to_normalise = ['dropouts_bve', 'dropouts_mvn', 'nc_t_bve', 'nc_t_mvn', 'ms_t_bve', 'ms_t_mvn', 'mgmteffort_mvn', 'mgmteffort_bve']
    for row in table: 
        for key in row:
...
if __name__ == '__main__':
    ...
    data = readFile(dirPath + 'MOAT_stage2.csv')
    data0 = destring(data)
    data1 = flipGinis(data0)
    data2 = rename_vars(data1)
    data3 = normalise(data2)
    success = split_dataset(data3)
    lookVal = lookup(data3)
